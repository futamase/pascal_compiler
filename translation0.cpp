...
int translate(std::vector<p1::Tetrad>& tetrads){
  ...
  // 式がひとつしかない場合
  if(tetrads.size() == 1){
    if(util::isDigit(tetrads.at(0).op_l)){
      int const_num = std::stoi(tetrads.at(0).op_l);
      if(const_num < -32768 || 32768 < const_num){
        auto label = Program::get_instance().push_const_label(const_num);
        Commander::get_instance().add_command({"load", "r0", label});
      }
      else{
        Commander::get_instance().add_command({"loadi", "r0", tetrads.at(0).op_l});
      }
    }
  }
  else{
