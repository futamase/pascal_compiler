...
int GetSym::get_char()
{
  char tmp;
  ist.get(tmp);
  register int c = tmp; // getc(infile);
  if (c == '\n')
  sline++;
  return c;
}

void GetSym::unget_char(register int c)
{
  if (c == '\n')
  sline--;
  // ungetc(c, infile);
...
void GetSym::flush_tokenbuf()
{
  tok.charvalue[0] = '\0';
  toklen = 0;
}

void GetSym::push_tokenbuf(int c)
{
  if (toklen < MAXIDLEN) {
...
// static void scan_symbol(int, int);
...
constexpr int MAX_HASH_VALUE = 19;
/*
13 keywords
15 is the maximum key range
*/

int GetSym::hash (register char *str, register int len)
{
...
struct GetSym::resword * GetSym::is_reserved_word  (register char *str, register int len)
{

  static resword wordlist[] =
  {
    { "", }, { "", }, { "", }, { "", }, { "", },
    {"do",  DO },
    { "", },
...
void GetSym::scan_symbol(int c, int k)
{
  tok.attr = SYMBOL;
